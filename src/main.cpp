...
#if defined(NDEBUG)
# error "Kumulcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x54e5eadd3b8ab44d1a840b67eaebf84ce1bafabe8e6d5937d1f8147702299a04");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kumulcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kumulcoin Signed Message:\n";
...
{
    // Kumulcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Kumulcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Kumulcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Kumulcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kumulcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kumulcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd2d02c786f44edaf2980011031447251471d717fd5af991c22b329a5256349d7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "First successful test of Kumulcoin - Developed By Christopher Vagalia";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d03d9e455445ec1b9889a879e952412c05f7194b822a785e0b9c29031615a1b23525eebe4e4917374daef90171b48ff3005cea224957cb3e3711f1e4da1b7c05") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524472521;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085265329;

        if (fTestNet)
        {
            block.nTime    = 1524472475;
            block.nNonce   = 386981800;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd72ad2933127f234405b9217d7587ce31752666bcddb36df1bdbd8b1d429cb68"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xff, 0xc2, 0xba, 0xd8 }; // Kumulcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KumulcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KumulcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KumulcoinMiner(CWallet *pwallet)
{
    printf("KumulcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kumulcoin-miner");
    ...
    try { loop {
        ...
        printf("Running KumulcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KumulcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KumulcoinMiner, pwallet));
}
